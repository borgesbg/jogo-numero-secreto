function verificaSeOChutePossuiUmValorValido(chute) {
    ...
    if (chuteForInvalido(numero)) {
        ...
        elementoChute.innerHTML += `<div>Número de tentativas ${tentativas}</div>`;
        tentativas ++
        return
    }
    ...
    if (numero === numeroSecreto) {
        if(tentativas >= 10){
            ...
            <div class="num-tentativas">Número de tentativas ${tentativas}</div>
            `
        }else{
            document.body.innerHTML =  `
            ...
            <h3>O número secreto era ${numeroSecreto}</h3>
            <button id="jogar-novamente" class="btn-jogar">Jogar Novamente</button>
            <div class="num-tentativas">Você tem sorte! Número de tentativas ${tentativas}</div>
            `
        }
